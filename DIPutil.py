...
def save_final_kernel_png(k, conf, gt_kernel, step=''):
    ...
    savepath_mat = os.path.join(conf.output_dir_path, 'kernel/%s.mat' % conf.img_name)
    savepath_png = os.path.join(conf.output_dir_path, 'png_kernel/%s_kernel.png' % conf.img_name)
    if step != '':
